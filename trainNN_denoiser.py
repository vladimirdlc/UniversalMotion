...
Y = np.load(fileChanged+"_noisy.npz")['clips']
...
#trainingData = qdata
trainingData, validationData = train_test_split(qdata, test_size=0.2, random_state=0)
x_train_noisy, x_test_noisy = train_test_split(qdataY, test_size=0.2, random_state=0)
...
idPrefix = '{}_k{}_hu{}_v{}_e{}_d{}_bz{}_valtest_denoise_activation{}'.format(fileChanged,kernel_size,hiddenUnits, version, epochs, dropoutAmount, batch_size, activationType)

plot_losses = PlotLoss(epochs, 'results/'+idPrefix)

print(trainingData.shape)
#print(validationData.shape)
history_callback = network.fit(x_train_noisy, trainingData, verbose=2,
                epochs=epochs,
                batch_size=batch_size,
                callbacks=[plot_losses],
                validation_data=(x_test_noisy, validationData))
